...
def index():
    ...
    if request.method == "POST":
        ...
        if password1 != password2:
            ...
            return render_template('home.html', message='Account created successfully! Please login to your account.')
...
def login():
    ...
    if request.method == "POST":
        ...
        if password == "admin":
            ...
            return render_template('admin_view.html',collection=collection)
...
def faculty():
    if "email" in session:
        ...
        return render_template('faculty.html', email=email, name=name,collection=collection, details_dict=details_dict)
    else:
        return redirect(url_for("login"))
    
@app.route('/faculty_view/<subject_id>', methods=["POST", "GET"])
def faculty_view(subject_id):
    if "email" in session:
        email = session["email"]
        user_data = records.find_one({"email": email})
        name = user_data['name']
        ...
        student_names = {}
        for student in details_collection.find():
            if subject_id in student:
                student_names[student.get("name", "Unknown")] = student[subject_id]
        
        # Filter student names where the value is greater than or equal to 35
        passed_students = {name: score for name, score in student_names.items() if int(score) >= 35}
        failed_students = {name: score for name, score in student_names.items() if int(score) < 35}
        total_students = len(student_names)
        pass_percentage = (len(passed_students) / total_students) * 100 if total_students > 0 else 0
        return render_template('faculty_view.html', email=email, name=name, subject_id=subject_id, p=passed_students,f=failed_students
,student_names=student_names,pass_percentage=pass_percentage, total_students=total_students)
    else:
        return redirect(url_for("login"))
